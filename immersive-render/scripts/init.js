...
Hooks.on(`ready`, () => {
    ...
    if (game.user?.isGM && game.settings.get('immersive-render', 'confirm00') === false) {
        // Initial Dialogue
        new Dialog({
            title: 'Welcome to Immersve Render (IR)',
            content: `<form>
                <h1>Welcome to Immersive Render (IR for short)</h1>
                <p>This module allows you (and any other GM) to configure folder paths for each Actor, Item and Journal in your world. From that folder path, a random audio file is picked to play when the entities sheet is rendered (opened) and closed.</p>
                ...
                <div class="form-group">
                    <label for="readIt">I have read the above: </label>
